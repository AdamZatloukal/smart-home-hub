...
esp_err_t data_colors_handler(httpd_req_t *req) {
    ...
    /* if the amount of data we received is larger than the buffer we
     will cutoff the amount of dara we received to the size of the buffer */
...
httpd_handle_t start_webserver() {
    ...
    */
    ...
    httpd_uri_t data_colors = {
        .uri = "/data/colors",
        .method = HTTP_POST,
        .handler = data_colors_handler,
    };
    ...
    if (httpd_start(&server_handle, &config) == ESP_OK) {
        ...
        ESP_ERROR_CHECK(httpd_register_uri_handler(server_handle, &data_colors));
    }
