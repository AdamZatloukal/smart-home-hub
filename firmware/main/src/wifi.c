...
esp_err_t connect_wifi()
{
    ...
    {                          // 0001 (WIFI_SUCCESS) & 0001 -> 0001 (true)
...
esp_err_t response_handler(esp_http_client_event_t *event)
{
    ...
    case HTTP_EVENT_ON_DATA:
        printf("Received %d bytes: %.*s\n", event->data_len, event->data_len, (char *)event->data);
        break;
    default:
        break;
    }
...
{
    /* POST */
    esp_http_client_set_url(client_handle, "http://" HOST_IP_PORT"/upload");
    esp_http_client_set_method(client_handle, HTTP_METHOD_POST); // says this is a post request
    esp_http_client_set_header(client_handle, "Content-type", "application/octet-stream");
    esp_http_client_set_post_field(client_handle, post_data, post_data_len); // sets the data we want to send
    /* performs all operations of the client - opening the connection, exchanging data, and closing the connection*/
    esp_err_t err = esp_http_client_perform(client_handle); // execute the request
    if (err == ESP_OK)
    {
        ESP_LOGI(TAG, "HTTP POST Status = %d, content_length = %" PRId64,
                 esp_http_client_get_status_code(client_handle),
                 esp_http_client_get_content_length(client_handle));
    }
    else
    {
